...
/**
 * Check if a runtime API is available on the connected chain.
 * Usage: `if (!hasRuntimeApi(api, 'commissionDashboardApi')) return null;`
 */
export function hasRuntimeApi(api: ApiPromise, apiName: string): boolean {
  const available = !!(api.call as any)?.[apiName];
  if (!available && typeof window !== 'undefined') {
    const callKeys = Object.keys((api.call as any) ?? {});
    console.warn(
      `[hasRuntimeApi] "${apiName}" not found on api.call. Available sections:`,
      callKeys,
    );
  }
  return available;
